# HIS API (智慧健康体检中心后端接口服务)

> 基于 Spring Boot 构建的 B2C 健康体检平台，涵盖⽤户端（商城 + 预约）与管理端（组织 / 业务 / 体检全流 程），实现从商品购买 → 体检预约 → 现场签到 → 医⽣检查 → 报告⽣成的完整闭环
...
- 🔥 安全防护：基于 Sa-Token 实现登录态管理与 RBAC 权限控制模型，支持细粒度接口鉴权，保障系统访问安全性
- 🔥 分布式并发控制：基于 Redisson 实现分布式锁，采用 tryLock + 自动续期 + 安全释放机制，有效防止重复提交与资源竞争，保障核心业务幂等性
- 🔥 实名核验体系：集成腾讯云人脸比对 + 静态活体检测 API，在签到环节校验身份证与现场人像一致性，提升业务安全等级
- 🔥 实时通信优化：基于 WebSocket + STOMP 实现双向通信，并设计连接鉴权 + 订阅权限校验机制，替代轮询方案，显著降低服务器压力并提升响应实时性
- 🔥 高并发排队调度：基于 Redis + Lua 实现原子化队列控制，结合 RabbitMQ 实现超时自动回收，解决高并发下排队人数不一致问题
- 🔥 异步解耦与任务调度：通过异步任务解耦报告生成流程，避免接口阻塞；结合 XXL-Job 定时清理过期数据，保障系统稳定性与数据一致性
- 🔥 数据库读写分离：基于 ShardingSphere-JDBC 构建一主双从 MySQL 集群，实现读写分离与负载均衡，降低主库压力并提升系统吞吐能力
- 🔥实时客服通信：前后台通过腾讯云 IM SDK + WebSocket  实现双向消息实时互通，保障客户服务响应时效。
